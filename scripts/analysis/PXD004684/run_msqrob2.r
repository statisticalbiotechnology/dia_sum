...
run_msqrob2 <- function(input, output, output_protein){
  peptidesFile = (input)
  ecols <- grep("_", names(read.delim(peptidesFile)))
  ...
  L <- makeContrast("conditionT=0", parameterNames = c("conditionT"))
  ...
  write.csv(msqrob2_df$conditionT, "msqrob2_results.csv")
...
input <- "LT_ST_Ctrl_msqrob2_input.csv"
output <- "LT_ST_Ctrl_msqrob2_output.csv"
output_protein <- "LT_ST_Ctrl_msqrob2_protein_output.csv"
