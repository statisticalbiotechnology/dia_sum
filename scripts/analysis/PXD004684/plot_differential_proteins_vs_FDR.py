...
def countProteins(df, method):
    ...
    count = df["Protein"].str.contains("").astype(int).copy(deep=True)
    df["differential_proteins_cumsum"] = count.cumsum()
    df["method"] = method
    return df[["differential_proteins_cumsum", "FDR", "method"]]
...
def get_differential_abundance_count(zipped_files, fc_threshold = 0.01, fdr_threshold = 1.00):
 ...
 #       df = df[df.FDR < fdr_threshold].copy()
        df_true = df[~df.Protein.str.contains("ENTRAP")]
        #df_false = df[df.Protein.str.contains("ENTRAP")]
        df_count = countProteins(df_true, method)
        #df_false_count = countProteins(df_false, "ENT_"+method)
        test.append(df)
        dfs.append(df_count)
        #dfs_false.append(df_false_count)
    res = pd.concat(dfs)
...
if __name__ == "__main__":
    ...
    #df_count["entrapment"] = df_count.method.map(lambda x:x.split("_")[0] == "ENT")
    #df_count["method_id"]  = df_count.method
    #df_count["method"] = df_count.method.map(lambda x:x.split("_")[-1])
