...
def msstats_get_clusters(ctrl_lt_file = "proteins.1vs2.tsv",
                 ctrl_st_file = "proteins.1vs3.tsv",
                 lt_st_file = "proteins.2vs3.tsv",
                 fdr_threshold = 0.05,
                 cluster_by_most_significant_protein = False):
    cols = ['adj.pvalue', 'log2FC', "upregulated"]
        
    ctrl_lt = pd.read_csv("Ctrl_LT_msstats_results.csv", sep = ",").rename({"Protein": "protein"}, axis = 1).set_index("protein")
    ctrl_st = pd.read_csv("Ctrl_ST_msstats_results.csv", sep = ",").rename({"Protein":"protein"}, axis = 1).set_index("protein")
    lt_st = pd.read_csv("LT_ST_msstats_results.csv", sep = ",").rename({"Protein":"protein"}, axis = 1).set_index("protein")
    
        # Fc thresholding procedure as in paper
    ctrl_lt = pd.concat([ctrl_lt[(ctrl_lt["log2FC"] > 1.5)],ctrl_lt[(ctrl_lt["log2FC"] < 0.67)]])
    ctrl_st = pd.concat([ctrl_st[(ctrl_st["log2FC"] > 1.5)],ctrl_st[(ctrl_st["log2FC"] < 0.67)]])
    lt_st = pd.concat([lt_st[(lt_st["log2FC"] > 1.5)],lt_st[(lt_st["log2FC"] < 0.67)]])


    ctrl_lt["upregulated"] = ctrl_lt["log2FC"] < 0
    ctrl_st["upregulated"] = ctrl_st["log2FC"] < 0 
    lt_st["upregulated"] = lt_st["log2FC"] < 0
    
    ctrl_lt = ctrl_lt[cols].rename({"adj.pvalue":"ctrl-lt:q_value", "upregulated":"ctrl-lt:upregulated", "log2(A,B)":"ctrl-lt:log2_fold_change"}, axis = 1)
    ctrl_st = ctrl_st[cols].rename({"adj.pvalue":"ctrl-st:q_value", "upregulated":"ctrl-st:upregulated", "log2(A,B)":"ctrl-st:log2_fold_change"}, axis = 1)
    lt_st = lt_st[cols].rename({"adj.pvalue":"lt-st:q_value", "upregulated":"lt-st:upregulated", "log2(A,B)":"lt-st:log2_fold_change"}, axis = 1)
...
C1, C2, C3, C4, C5, C6 = msstats_get_clusters(ctrl_lt_file = "Ctrl_LT_msstats_results.csv",
                                      ctrl_st_file = "Ctrl_ST_msstats_results.csv",
                                      lt_st_file = "LT_ST_msstats_results.csv")

protein_table = protein_count_table(C1, C2, C3, C4, C5, C6)
count_table = pd.DataFrame(protein_count_table(C1,C2,C3,C4,C5,C6).sum(), columns = ["MSstats"])
count_table.to_csv("msstats_protein_count.tsv", sep = "\t")
...
df.to_csv("enrichr_msstats_whole_bgr.tsv", sep = "\t", index = False)
