...
def calibration_plot(df, output, xlim = [0,0.10], ylim = [0,0.20]):
    ...
    axs.tick_params(axis='y', which='major', labelsize=28)
    axs.set_xlim(xlim)
    axs.set_ylim(ylim)
...
def get_fraction_hela_df(triqler, top3, msstats, msqrob2, limma):
    fdrs = []
    for df, method in zip([triqler, top3, msstats, msqrob2, limma], ["Triqler", "Top3", "MSstats", "MSqRob2", "Limma"]):
...
def main(triqler_file, top3_file, msstats_file, msqrob2_file, limma_file, output, fc_threshold = 0, xlim = [0,0.10], ylim = [0,0.2]):
    ...
    msqrob2.rename({"protein":"Protein"}, axis = 1, inplace = True)
    limma = pd.read_csv(limma_file, sep = "\t").rename({"adj.P.Val":"FDR", "P.Value":"p", "Unnamed: 0": "Protein", "logFC":"log2FC"}, axis = 1)
    ...
    if fc_threshold != 0:
        ...
        limma = threshold_fc(limma, fc_threshold)
        
    df = get_fraction_hela_df(triqler, top3, msstats, msqrob2, limma)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--limma_input', type=str,
                        help='Limma results file.')
