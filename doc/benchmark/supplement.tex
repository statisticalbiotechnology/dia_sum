...
\begin{figure}[hbt]
    ...
    \end{tabular}
    \caption{{\bf The compared methods' ability to differentiate differentially abundant proteins.} We plotted the number of reported differentially abundant  (A,D) {\em E. Coli}, (B,E) Yeast proteins and (C,F) the sum of them as a function of the number of proteins from the HeLa background when sorting according to significance for (A-C) ID pipeline and (D-F) PS pipeline. For the test, we selected a fold-change evaluation of 0.51 for Triqler and fold-change threshold of 0.51 for Top3, MSstats, and MSqRob2. \label{fig:ability_to_differentiate_differentially_abundant_specie_vs_hela}}
\end{figure}
...
\begin{figure}[hbt]
    ...
    \todo[inline]{This is the results for where we ran OpenSwath with a spectral library that is not reduced}
\end{figure}
