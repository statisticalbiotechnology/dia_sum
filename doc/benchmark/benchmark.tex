...
\begin{figure}[htp]
    ...
    \todo[inline]{Is limma included the right way?}    
\end{figure}
...
We downloaded the dataset PXD031322 from PRIDE. The study consist of a control group mice, and mice injected with oxaliplatin (OXA) after one week (referred to as short-term or ST) and after two weeks (referred to as long-term or LT). We restricted ourselves to the control-LT segment of the study to test our protein summarization tools. Further details about the study is available through Yang et al. ~\cite{YANG2022104682}. The \verb|.wiff| files were converted using msconvert (using Windows OS msconvert version 3.0) with peakPicking filter msLevel=1-). For this study the supplied spectral library from the PRIDE was used. Pseudo-reverse sequences was added to the database as decoys using OpenSwathDecoyGenerator. This was required for DIA-NN to give decoy proteins in the output report.tsv file. The decoy proteins was required to run Triqler.  
...
\todo{Patrick, does the decription below cover what you did? Also is there a command line option that you used for estimating this threshold. ANS: This is correct, PS pipeline -np.log() and ID pipelin -np.log10() was used on the searchScore, I don't think there was any particular reason i did this. LK: Nej, det är stycket härunder vi pratrar om. Kan du säga om jag beskriver metoden för ett lower-bound treshold är korrekt?}
