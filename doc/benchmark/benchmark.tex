...
\begin{figure}[hbt]
    ...
    \begin{tabular}{cc} 
        ...
        A & B
    \end{tabular}
  \caption{{\bf Comparison of calibration of the compared summarization methods.} We plotted the fraction of reported differentially abundant HeLa proteins as a function of $q$~value threshold for (A) ID spectral library pipeline and (B) PS spectral library pipeline. The multiple test correction is performed with the $q$~value for Triqler and Top3, while Benjamini-Hochberger corrections were used for MSstats and MSqRob2. \label{fig:frac_hela_vs_fdr}}
\end{figure}
