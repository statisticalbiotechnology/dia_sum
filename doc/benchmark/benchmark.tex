...
We have previously designed a hierarchical Bayesian model, Triqler, able to control for errors from both the identification and quantification process in LFQ experiments\cite{The2018Integrated}. Triqler weights quantitative peptide data with thier respective certainty scores to get error estimates for proteins. Furthermore, by using a probabilistic graphical model Triqler induces less bias by avoiding missing value imputation and manages to integrate peptide values to proteins posteriors. These protein posteriors are then integrated between the samples to get a single differential abundance false discovery rate (FDR) that reflects the uncertainties in identification and quantification. Triqler was designed for handling LFQ data from Data-dependent acquisition (DDA). However, many labs prefer Data-independent acquisition (DIA) mass spectrometry \cite{venable2004automated} as they find that it gives more reproducible peptide detection, and allows for a broader dynamical range in quantification \cite{bern2010deconvolution,zhang2020DIA}, compared to DDA. 
...
However, when Triqler tried to estimate the distribution of missing values, we noted that the lower number of missing values in DIA compared to DDA (<2\% for DIA), lead to parameter estimates out of the expected range. We hence introduced an alternative estimation procedure for DIA data (See Supplementary Figure \ref{fig:fraction_missing_values}) that can be enabled by a command line option in Triqler. 
