...
\todo{I dont like abreviations of abreviations, could we change the name to ID instead?}
...
We implemented a short script that for each protein selected the average of the 3 most abundant PSMs for each protein and sample. In samples only having two PSMs available these were also included, still represented by their average, however, proteins with one or zero PSMs per sample were excluded. \todo{You previously wrote "samples having zero or one PSM for the protein were excluded." I guess my edit is right? Please confirm.} PSMs were filtered at an 1\% FDR before performing the Top3 protein summarization. 
