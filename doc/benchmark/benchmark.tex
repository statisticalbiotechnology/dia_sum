...
\subsubsection*{Comparison of differentially abundant proteins between methods}
To access the similarity between the methods we compared the methods' reported log2-fold change ratios with scatterplots. In figure \ref{fig:scatter_triqler_vs_method}A the dispersion in y-axis shows that Top3 has more proteins that deviate from the true log2-fold change than Triqler. Figure \ref{fig:scatter_triqler_vs_method}B and \ref{fig:scatter_triqler_vs_method}C show that the \textit{E. coli.} log2-fold changes are negatively biased in MSstats and MSqRob2 relative to Triqler. A comparison of both pipelines is shown in \ref{fig:fc_scatter_triqler_vs_method_supplement}.
...
\begin{figure}[hbt]
    ...
    \begin{tabular}{ccc} 
        ...
        A & B & C
    \end{tabular}
  \caption{{\bf Comparison of reported log2-fold change between different methods.} We plotted the log2-fold change of Triqler against (A) Top3, (B) MSstats and, (C) MSqRob2 for the ID pipeline. Dots arranged across the diagonal line indicate that Triqler and the compared method have an identical log2-fold change and dots deviating from the diagonal line indicate a difference in log2-fold estimation. Only log2-fold changes of proteins from both Triqler and the compared methods are included in the scatter plots. \label{fig:scatter_triqler_vs_method}}
\end{figure}
