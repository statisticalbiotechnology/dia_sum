...
For the SL workflow we searched the LFQBench provided DDA runs with MSFragger\cite{} with a precursor tolerance of FIXME ppms and constructed a spectral library with EasyPQP \cite{} to from the DDA-search results using a PSM-level FDR treshold of \todo{which level did you use?}. The spectral library was subsequently matched to the DIA data with the OpenSwath Workflow. PyProphet\cite{} was used for statistical validation and TRIC\cite{} for feature alignment. This resulted in a set of detected peptides together with their assesed peptide identification accuracues and abundance estimates.

\subsubsection{Pseudo Spectrum generation (PS)}

 For the PS workflow we also used the fragpipe software, employing DIA-Umpire to extract pseudo-spectra from the DIA data. The pseudo-spectra were subsequently searched using using MSFragger with a precursor tolerance of FIXME ppms and allowing for FIXME variable modifications. A spectral library was build from the resulting PSMs at a PSM-level treshold of FIXME using easyPQP that was used for peptide quantifications with DIA-NN. The DIA-NN uses a mProphet-based algorithm for statistical validation \cite{reiter2011mprophet} \cite{demichev2020dia}. \todo{I do not get this. What is mProphet used for at this step?} 
...
\[ \text{censnorm}(x) = \left( \frac{1}{2} + \frac{1}{2}\right) \text{tanh}\left( \frac{x - \mu_m}{\sigma_m}\right) \mathcal{N}(x | \mu_f, \sigma_f)\]
 ...
 The intuition is that low ion currents are harder to detect than high ion currents \cite{The2018Integrated}. Triqler integrated this distribution rather than drawing a single point estimate from this. 
...
\ref{fig:missingness_fraction} show the missingness proportions for OSW and DIANN processed runs. 

%* missing values [ok]
%* missing value model [in progress]
%* previous missing value models []
%* censored data []
%* benefit of triqler for missing values []
