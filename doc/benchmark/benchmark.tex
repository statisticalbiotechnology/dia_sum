...
We applied Triqler, MSstats, MSqRob2, and Top3(see the Methods section) to the peptide abundances derived from the ID and PS workflows from the varying concentrations of {\em E. Coli} and yeast concentrations in a background of HeLa-cells of the LFQBench set. 

Triqler is not meant as a tool to give point estimates of abundances, instead triqler is suited for calculating distributions of protein fold changes. However, to make a usefull comparison between methods, one can use the Triqler's maximum posteriors (MAP) as point estimates of protein abundance. To get an overview of the results of the methods we plotted histograms of the reported protein level fold-changes by the compared methods as well as Triqler's MAPs as reported  in Figure \ref{fig:fc_histogram}. In the histograms, we included lines indicating the actual fold change of the pipetted whole-cell lysate concentrations as a reference.
We observe that Triqler and Top3 have less bias than MSstats and MSqRob2, by the qualitative observation that the apex of the distributions is centered more closely to the lysate mixture rates. MSstats and MSqRob2 have distribution apexes that for each specie are slight to the left of the true lysate mixture rates. We also observe that the apex of the distributions has higher values for Triqler than the other methods. We also observe in Figure \ref{fig:fc_scatter} that the regression line goes towards the true fold-change values for Triqler, while it remains constantly postive for Top3, MSstats and MSqRob2. We also observe that Triqler has longer tails for the estimated yeast and {\em E. Coli} fold change distributions. These values are explained by proteins quantified with fewer peptides, i.e. less certain protein quantification. A figure of number of peptides per protein in different fold change regions can be seen in the supplement \ref{fig:number_of_peptides_supplement}.
...
\begin{figure}[hbt]
    ...
    \begin{tabular}{cc}
        ...
        C & D 
    \end{tabular}
    \caption{{\bf Protein-level benchmark results} from (A) Triqler, (B) Top3, (C) MSstats, and (D) MSqRob2. A regression fit is marked by the dashed line. The results for Triqler indicates that the regression line beta is negative for the yeast and positive for the \it{E.Coli}. This hows that as we have more signal we have a beta tilted towards the true values, while Top3, MSstats and MSqRob2 has a positive beta regardless of specie (see Supplementary Figure \ref{fig:fc_scatter_supplement} for protein-level results for both ID and PS workflows). \label{fig:fc_scatter}}
\end{figure}
